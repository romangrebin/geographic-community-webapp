 ...
 */
async function createRepository(): Promise<CommunityRepository> {
  const override = process.env.COMMUNITY_ADAPTER

  if (override === 'mock') {
    const { MockCommunityRepository } = await import('./adapters/mock')
    return new MockCommunityRepository()
  }

  if (override === 'supabase' || process.env.NEXT_PUBLIC_SUPABASE_URL) {
    const { SupabaseCommunityRepository } = await import('./adapters/supabase')
    return new SupabaseCommunityRepository()
  }

  // Local dev default: file-backed persistence that survives restarts.
  const { JsonFileCommunityRepository } = await import('./adapters/json-file')
...
let _dbPromise: Promise<CommunityRepository> | null = null

export function getDb(): Promise<CommunityRepository> {
  if (!_dbPromise) {
    _dbPromise = createRepository()
  }
  return _dbPromise
}

// For backwards compatibility — a proxy that lazily resolves.
// Most callers should migrate to `getDb()` over time.
export const db: CommunityRepository = new Proxy({} as CommunityRepository, {
  get(_target, prop: string) {
    return async (...args: unknown[]) => {
      const repo = await getDb()
      return (repo as unknown as Record<string, (...a: unknown[]) => unknown>)[prop](...args)
    }
  },
})
